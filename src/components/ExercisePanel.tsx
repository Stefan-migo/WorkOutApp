...
export function ExercisePanel({
  ...
  const chipClass =
    'px-3 py-1 rounded-full bg-surface/20 text-timer-on font-label-caps text-label-caps border border-timer-border/50'
  ...
  if (!name) {
    return (
      <div className="glass-panel-dark rounded-xl p-16 w-full max-w-lg flex flex-col items-center text-center">
        ...
        <p className="font-body-md text-body-md text-timer-muted mt-8">
  ...
  return (
    <div className="glass-panel-dark rounded-xl p-16 w-full max-w-lg flex flex-col">
      {/* Image Gallery */}
      <div className="w-full h-64 bg-accent rounded-lg mb-16 overflow-hidden relative border border-timer-border/25 flex items-center justify-center">
        {displayImage ? (
          ...
          {galleryImages.map((img, idx) => (
            <button
              ...
              className={`w-10 h-10 rounded-md overflow-hidden border-2 transition-colors ${
                idx === selectedImage ? 'border-timer-on' : 'border-timer-border hover:border-timer-border'
              }`}
            >
      ...
      {/* Name */}
      <h2 className="font-headline-lg text-headline-lg text-timer-on mb-xs">{name}</h2>

      {/* Description */}
      {description && <p className="font-body-md text-body-md text-timer-muted mb-16">{description}</p>}
      ...
      {primaryMuscles && primaryMuscles.length > 0 && (
        <div className="mb-8">
          <p className="font-label-caps text-label-caps text-timer-muted mb-xs">Primary Muscles</p>
      ...
      {secondaryMuscles && secondaryMuscles.length > 0 && (
        <div className="mb-8">
          <p className="font-label-caps text-label-caps text-timer-muted mb-xs">Secondary Muscles</p>
          <div className="flex flex-wrap gap-2">
      ...
      {equipment && equipment.length > 0 && (
        <div className="mb-8">
          <p className="font-label-caps text-label-caps text-timer-muted mb-xs">Equipment</p>
          <div className="flex flex-wrap gap-2">
            {equipment.map((e) => (
      ...
      {instructions && instructions.length > 0 && (
        <div className="mt-8">
          <button
            ...
            className="flex items-center gap-2 text-timer-on font-label-caps text-label-caps hover:text-timer-muted transition-colors"
          >
          ...
          {showInstructions && (
            <ol className="flex flex-col gap-4 mt-8 list-decimal list-inside">
              {instructions.map((step, idx) => (
                <li key={idx} className="font-body-md text-body-md text-timer-muted">{step}</li>
              ))}
            </ol>
          )}
        </div>
